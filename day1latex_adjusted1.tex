...
\newpage 
 ...
 \begin{figure}[!ht] 
     ...
     \includegraphics[width=0.52\textwidth]{./figures/prof_tend.pdf}
        \caption[Diagrams and \textbf{plots}]{Here we combine a \textbf{Tikz} diagrams and images. Note that there the references within the diagram include active hyperlinks to other part of this document.}    
      \label{fig:figures}    
 \end{figure}
 
\subsection{Picture}
The figure above is missing labels for some of the panels. Also, we would like to move the diagram on the left little bit higher and the one right little bit lower. Instead of opening the figures in some graphical editor, we can perform these tasks in \LaTeX with the environment \texttt{picture}. We start the picture environment and encapsulate each panels in the \texttt{put} command, which in round brackets defines the coordinates where each panel is located. The postion is given with respect to the bottom left corner (unless specified otherwise). You can see the result in figure \ref{fig:picture}. 
\lstinputlisting[ language={[latex]tex},      % tex syntax, latex option
  caption={Tikz Latex code for Figure \ref{fig:picture}a }, label={code:tikz}, 
  firstline=367, firstnumber=1,  lastline=384,
...
\begin{figure}[h]
 ...
 \put(  0,  20){                       % left panel shifted up by 20 (mm, points, etc.)
     \input{./latex/diagram.tex}       % insert the diagram as first panel
 }
 \put(170,  0){                        % right panel shifted right by 160 (mm,
     \includegraphics[width=0.52\textwidth]{./figures/prof_tend.pdf}
 }
...
\lstinputlisting[ language={[latex]tex},      % tex syntax, latex option
  caption={Tikz Latex code for Figure \ref{fig:picture}a }, label={code:diagram}, 
...
\lstinputlisting[ language={[latex]tex},      % tex syntax, latex option
  caption={Tikz Latex code for Figure \ref{fig:picture}b }, label={code:flowchart}, 
...
\begin{itemize}
 \item \href{https://www.ctan.org/pkg/latexdiff}{Latexdiff package page}
 \item \href{https://texdoc.org/serve/latexdiff/0}{Latexdiff guidebook}
 \item \href{https://texra.ai/guide/latex-diff.html}{Advanced Latexdiff guidelines}
\end{itemize}
